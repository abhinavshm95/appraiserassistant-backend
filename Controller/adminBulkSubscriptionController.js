 ...
 * Admin only - one-time payment for multiple subscription codes
 */
const createBulkCheckoutSession = async (req, res, next) => {
    try {
        ...
        // Get admin's Stripe customer ID or create one
        let userSubscription = await Model.userSubscription.findOne({ userId: admin._id });
        let stripeCustomerId;

        if (userSubscription && userSubscription.stripeCustomerId) {
            stripeCustomerId = userSubscription.stripeCustomerId;
        } else {
            // Create a new Stripe customer for the admin
            ...
            if (userSubscription) {
                ...
                await Model.userSubscription.create({
                    userId: admin._id,
        ...
        const subscriptionDurationDays = getSubscriptionDurationDays(
            priceRecord.interval,
            priceRecord.intervalCount
        );

        // Create subscription checkout session (recurring billing)
        // No database entry created here - will be created by webhook on successful subscription
...
/**
 * Get all bulk subscription purchases for the admin
 */
const getBulkPurchases = async (req, res, next) => {
    try {
        ...
        const [purchases, total] = await Promise.all([
            Model.bulkSubscriptionPurchase.find(query)
                .sort({ createdAt: -1 })
                .skip(skip)
...
const createAdminPortalSession = async (req, res, next) => {
    try {
        ...
        const userSubscription = await Model.userSubscription.findOne({ userId: admin._id });
