...
const register = async (req, res, next) => {
  try {
    ...
    const hashPassword = await universalFunction.securePassword(req.body.password);
...
const login = async (req, res, next) => {
  try {
    ...
    const password = await universalFunction.verifyPassword(req.body.password, user.password);
    if (!password) {
      return res.status(400).json({
...
const changePassword = async (req, res, next) => {
  try {
    ...
    const comparePassword = await universalFunction.verifyPassword(req.body.password, user.password);
    if (!comparePassword) {
      return universalFunction.errorFunction(
        req,
        res,
    ...
    const hashPassword = await universalFunction.securePassword(req.body.newPassword);
...
const listUsers = async (req, res, next) => {
  try {
    ...
    const [users, total] = await Promise.all([
      Model.user.find(query).select("-password -otp -accessToken").skip(skip).limit(limit).sort({ createdAt: -1 }),
...
const getUserById = async (req, res, next) => {
  try {
    const user = await Model.user.findById(req.params.id).select("-password -otp -accessToken");
    ...
    const subscription = await Model.UserSubscription.findOne({ userId: user._id });
...
const toggleUserActivation = async (req, res, next) => {
  try {
    ...
    // If deactivating, we can also invalidate tokens (optional, but good practice)
    if (newStatus === false) {
      ...
      await Model.user.updateOne(
        { _id: user._id },
        { $set: { isActive: newStatus, sessionVersion: (user.sessionVersion || 0) + 1 } },
      );
    } else {
    ...
    return universalFunction.successFunction(
      ...
      { _id: user._id, isActive: newStatus },
    );
  } catch (err) {
    next(err);
  }
...
const getProfile = async (req, res, next) => {
  try {
    const user = await Model.user.findOne({ email: req.user.email }).select("-password -otp -accessToken");
...
const updateProfile = async (req, res, next) => {
  try {
    ...
    const updatedUser = await Model.user
      .findOneAndUpdate({ email: req.user.email }, { $set: updates }, { new: true })
