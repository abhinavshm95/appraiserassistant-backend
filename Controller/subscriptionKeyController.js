...
const getAllCodes = async (req, res, next) => {
  try {
    ...
    // If source filter is applied, we need to filter in memory since purchaseId is populated
    // Or better yet, we can't easily filter by populated field in mongo without aggregation
    // So we'll fetch purchases first if source filter is present

    let validCodeIds = null;
    if (status === "all" && req.query.source) {
      // Logic if only source filter matches
      const source = req.query.source;
      let purchaseQuery = {};
      if (source === "admin") {
        purchaseQuery.totalAmount = 0;
      } else if (source === "purchased") {
        purchaseQuery.totalAmount = { $gt: 0 };
      }

      if (Object.keys(purchaseQuery).length > 0) {
        const purchases = await Model.bulkSubscriptionPurchase.find(purchaseQuery).select("_id");
...
const revokeCode = async (req, res, next) => {
  try {
    ...
    const subscriptionCode = await Model.bulkSubscriptionCode.findById(codeId).populate("redeemedBy", "name email");
    ...
    if (wasRedeemed) {
      ...
      if (previousUser) {
        await Model.userSubscription.findOneAndUpdate({ userId: previousUser._id }, { status: "canceled" });
      }
    } else {
...
const reactivateCode = async (req, res, next) => {
  try {
    ...
    const subscriptionCode = await Model.bulkSubscriptionCode
      .findById(codeId)
      .populate("previouslyRedeemedBy", "name email");
...
const exportCodes = async (req, res, next) => {
  try {
    ...
    const codes = await Model.bulkSubscriptionCode
      .find(query)
      .sort({ createdAt: -1 })
      .populate("redeemedBy", "name email")
      .populate("previouslyRedeemedBy", "name email")
