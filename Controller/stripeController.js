...
const createCheckoutSession = async (req, res, next) => {
  try {
    ...
    // Admins and Managers cannot buy individual subscriptions
    if (["admin", "manager"].includes(user.role)) {
      return universalFunction.errorFunction(
        req,
        res,
        code.statusCodes.STATUS_CODE.FORBIDDEN,
        "Admins and Managers cannot purchase individual subscriptions",
      );
    }
...
const getProducts = async (req, res, next) => {
  try {
    ...
    const productsWithPrices = products.map((product) => {
      const productPrices = prices.filter((price) => price.stripeProductId === product.stripeProductId);
      return {
        ...product.toObject(),
